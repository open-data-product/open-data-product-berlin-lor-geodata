...
def load_data(source_path, results_path, clean=False, quiet=False):
    ...
    for subdir, dirs, files in os.walk(source_path):
        for file_name in files:
            ...
            target_file_name = get_target_file_name(file_name)

            # Make results path
            os.makedirs(os.path.join(results_path, subdir), exist_ok=True)

            source_file_path = os.path.join(source_path, subdir, file_name)
            results_file_path = os.path.join(results_path, subdir, target_file_name)
            ...
            if clean or not os.path.exists(results_file_path):
                ...
                with open(results_file_path, "w", encoding="utf-8") as geojson_file:
                    ...
                    if not quiet:
                        print(f"✓ Load {target_file_name}")
            else:
                print(f"✓ Already exists {target_file_name}")
